...
const BirthdayWidget: React.FC = () => {
    ...
    const birthdayBoys = users.filter(u => u.role === 'MEMBER' && isBirthdayToday(u.dob));

    const sendWhatsApp = (member: User) => {
        const text = encodeURIComponent(`Happy Birthday ${member.employeeName}! 🎂 Wishing you a fantastic day and a great year ahead. Best regards, IPREU Union.`);
        window.open(`https://wa.me/${member.mobileNumber}?text=${text}`, '_blank');
    };

    if (birthdayBoys.length === 0) return null;

    return (
        <div className="bg-gradient-to-br from-orange-50 to-white p-8 rounded-[2rem] border-2 border-orange-200 shadow-xl relative overflow-hidden">
            <div className="absolute -right-4 -top-4 w-24 h-24 bg-orange-200/20 rounded-full blur-xl animate-pulse"></div>
            <div className="flex items-center gap-4 mb-6">
                <div className="p-3 bg-orange-600 rounded-xl text-white shadow-lg">
                    <span className="text-xl">🎂</span>
                </div>
                <div>
                    <h3 className="text-xl font-black text-[#002316] uppercase tracking-tight">Birthdays Today</h3>
                    <p className="text-[10px] font-bold text-orange-600 uppercase tracking-widest mt-1">Found {birthdayBoys.length} Celebrants</p>
                </div>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {birthdayBoys.map(m => (
                    <div key={m.id} className="bg-white p-4 rounded-2xl border border-orange-100 flex items-center justify-between group hover:border-orange-500 transition-all">
                        <div className="flex items-center gap-3">
                            <div className="w-12 h-12 rounded-full border-2 border-orange-100 overflow-hidden bg-orange-50">
                        ...
                        <button 
                            ...
                            className="p-2.5 bg-emerald-50 text-emerald-600 rounded-xl hover:bg-emerald-600 hover:text-white transition-all shadow-sm flex items-center gap-2"
                        ...
                        >
                            ...
                            <span className="text-[10px] font-black uppercase">Send Greeting</span>
                        </button>
                    </div>
                ))}
            </div>
...
const AdminDashboard: React.FC<AdminDashboardProps> = ({ activeTab, setActiveTab }) => {
    ...
    const filteredMembers = approvedMembers.filter(m => {
        const matchesSearch = m.employeeName?.toLowerCase().includes(searchTerm.toLowerCase()) ||
                             m.employeeNumber?.toLowerCase().includes(searchTerm.toLowerCase()) ||
                             m.email?.toLowerCase().includes(searchTerm.toLowerCase());
        const matchesLocation = filterLocation === 'ALL' || m.postingLocation === filterLocation;
